...
- Experience uses a single bordered rounded frame like Achievements, without FlickeringGrid or vertical timeline connectors. Rows use a 1:3 metadata-to-description grid from `md`, stack on mobile, and show the first three roles before the bottom `Show More` control reveals the rest.
...
- Profile card: the Dither canvas fills its full card-region wrapper instead of its default intrinsic bitmap size.
